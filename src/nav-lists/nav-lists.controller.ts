import { Controller, Get, Post, Body, Patch, Param, Delete, UseGuards, HttpException, HttpStatus, ParseIntPipe, Inject, Logger, Req, Query } from '@nestjs/common';
...
export class NavListsController {
  ...
  @ApiQuery({ name: 'idioma', required: false, type: 'string', description: 'Idioma (es, en, ca)', example: 'es' })
  ...
  async getContainers(@Param('entity') entity: string, @Query('idioma') idioma: string = 'es', @Req() req: Request) {
    this.logger.log(`Se han solicitado los contenedores para la entidad: ${entity} con idioma: ${idioma}`)
    try {
      ...
      // Llamar al servicio y pasar si tiene el permiso y el idioma
      return await this.navListsService.getContainersByEntity(entity, hasUserViewPermission, idioma);
    } catch (error) {
